...
if (fs.existsSync(DATA_FILE)) {
  try {
    ...
    console.error("Erreur lors du chargement de userdata.json");
  }
}
...
function canUseDispatch(member) {
  return (
    ...
    hasRole(member, CAPORAL_ROLE_ID) ||
    hasRole(member, COMMANDANT_ROLE_ID) ||
    hasRole(member, HAUT_GRADE_ROLE_ID)
  );
}
...
function getAgent(user) {
  ...
  userdata.agents[user.id].username = user.username;
...
client.once("ready", async () => {
  ...
  setInterval(checkSundayReport, 60 * 1000);
});
...
client.on("interactionCreate", async interaction => {
  ...
  if (command === "clockin") {
    ...
    if (!hasRole(member, LSPD_ROLE_ID)) {
      return interaction.reply({
        content: "❌ Vous devez avoir le rôle LSPD pour prendre votre service.",
        ephemeral: true
      });
    }
  ...
  if (command === "clockout") {
    ...
    const service = services.get(user.id);
  ...
  if (command === "dispatch") {
    ...
    if (!canUseDispatch(member)) {
      return interaction.reply({
        content: "❌ Vous n'avez pas le rôle nécessaire pour prendre le dispatch.",
        ephemeral: true
      });
    }
    ...
    if (dispatchers.has(user.id)) {
      return interaction.reply({
        content: "⚠️ Vous êtes déjà au dispatch.",
        ephemeral: true
      });
    }
  ...
  if (command === "dispatchout") {
    ...
    if (!dispatchers.has(user.id)) {
      return interaction.reply({
        content: "❌ Vous n'êtes pas au dispatch.",
        ephemeral: true
      });
    }
  ...
  if (command === "addminute") {
    ...
    if (minutes <= 0) {
      return interaction.reply({
        content: "❌ Le nombre de minutes doit être supérieur à 0.",
        ephemeral: true
      });
    }
...
async function sendWeeklyReport() {
  ...
  try {
    ...
    if (!channel) {
      console.error("Canal du bilan introuvable.");
      return;
    }

    const agents = Object.values(userdata.agents);
    ...
    console.log("Bilan hebdomadaire envoyé.");
  }
