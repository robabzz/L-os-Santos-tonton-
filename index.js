...
async function setupTicket(
  ...
  return interaction.reply({
    content:
      `✅ Le système ${ticketSystem} a été installé dans ce salon.`,
    ephemeral: true
  });
}
...
async function createTicket(
  ...
  const existing =
    guild.channels.cache.find(
      channel =>
        ...
        channel.topic ===
          `ticket:${interaction.user.id}:${ticketSystem}`
    );
  ...
  await ticketChannel.send({
    content:
      `${interaction.user} ${type.roles.map(role => `<@&${role}>`).join(" ")}`,
    embeds: [embed],
    components: [row]
  });
