...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("userdata")
    .setDescription("Voir ses statistiques LSPD"),

  new SlashCommandBuilder()
    .setName("dispatch")
    .setDescription("Générer le dispatch des unités"),
...
function canUseServiceCommands(member) {
  return (
    hasRole(member, LSPD_ROLE_ID) ||
    hasRole(member, EMS_ROLE_ID)
  );
}
...
function canManageServices(member) {
  return (
    ...
    hasAnyRole(
      member,
      EXTRA_HAUT_GRADE_ROLES
    )
  );
}

function canSetupTickets(member) {
  return hasAnyRole(
    member,
    SETUP_TICKET_ROLES
  );
  ...
  async interaction => {
    try {
      ...
      if (
        ...
        if (
          !canUseServiceCommands(member)
        ) {
          return interaction.reply({
            content:
              "❌ Vous devez avoir le rôle LSPD ou EMS pour prendre votre service.",
            ephemeral: true
          });
        }
      ...
      if (
        ...
        if (
          ...
          return interaction.reply({
            content:
              "❌ Vous devez avoir le rôle LSPD ou EMS pour terminer votre service.",
            ephemeral: true
          });
        }
      ...
      if (
        ...
        if (
          ...
          return interaction.reply({
            content:
              "❌ Vous devez avoir le rôle LSPD ou EMS pour consulter vos statistiques.",
            ephemeral: true
          });
        }
      ...
      if (
        ...
        if (
          ...
          return interaction.reply({
            content:
              "❌ Vous devez avoir le rôle LSPD ou EMS pour utiliser le dispatch.",
            ephemeral: true
          });
        }
...
async function sendWeeklyReport() {
  ...
  try {
    ...
    if (!REPORT_CHANNEL_ID) {
      console.error(
        "❌ REPORT_CHANNEL_ID n'est pas configuré."
      );
